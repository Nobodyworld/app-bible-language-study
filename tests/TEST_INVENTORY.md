...
| `tests/desktop-staging.mjs` | Full-corpus split inventory, deterministic staged paths, exclusions including retired poll seeds/fixtures, CSP transform, symlink rejection evidence, and measured frontend/resource bytes. |
...
| `app/tools/run-desktop-e2e.mjs` | Actual debug native binary launch, Proverbs 1:1 Language Study and Strong's, exact source-token Favorite and Meaning writes, native persistence observation, historical poll merge/replace with 605 events, retired poll/job UI absence, clean relaunch after each import, route restoration, and persisted controls with captured logs/screenshots. Test data roots and injected error capture are debug/E2E-only and absent from release behavior. |
