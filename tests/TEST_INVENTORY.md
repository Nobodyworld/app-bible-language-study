...
| `app/scripts/physical-pack-interaction-test.mjs` | Edge desktop, portrait, narrow, mobile-width, mobile-device, light/dark, and reduced motion | Distribution-aware bundled tools and fallback; managed override; strict unavailable states; visible storage plans; pre-staging quota rejection; unavailable-estimate success; plan/cancel equality and focus; install/offline reads; reload missing/digest/length reconciliation; repair; update; valid/invalid rollback recovery; removal fallback; exact route/selection/highlight/reader-scroll/detail-history/lock/detail-scroll preservation; containment; and zero console/page/request/HTTP errors. |
